...
buckets_size = [1000, 1000, 1000, 862]
nb_epoch = 150
...
def custom_loss(y_true, y_pred):
    ...
    y1 = K.reshape(y_pred, (batch_size, nb_frame, S, S, nb_features))
    y2 = K.reshape(y_true, (batch_size, nb_frame, S, S, nb_features))
    ...
    loss = K.sum(K.flatten(loss_probs + lambda_coord * loss_coords + lambda_conf * loss_conf))
...
# w_path = lstm_weights_directory + 'checkpoint-000.hdf5'
w_path = None
...
def batchGenerator():
    while 1:
        (X_train, y_train, _, _, _) = get_data(image_net_directory, batch_size, nb_frame, 'train', synset_wnet2id, bucket_id = buckets_id[0])
...
# lstm_network.fit_generator(batchGenerator, samples_per_epoch = buckets_size[0] / batch_size, nb_epoch = nb_epoch, verbose = 1, callbacks = callbacks)

###########################################
# Testing the model and show some results #
###########################################

(X_train, y_train, image_paths, label_paths, indexes) = get_data(image_net_directory, 1, nb_frame, 'val', synset_wnet2id, verbose = False)
