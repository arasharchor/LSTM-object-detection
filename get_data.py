...
def get_x_frame(path, frame_id, wrg, hrg):
...
def get_y_frame(path, frame_id, wrg, hrg, synset_wnet2id):
    label_path = path + frame_id + '.xml'

    folder, filename, database, width, height, objects = parse_ILSVRCXML(label_path)
    y = process_objects(int(width), int(height), objects, wrg, hrg, synset_wnet2id)

    return y

def get_x(path, snippet_id, ind, nb_frame, wrg, hrg, type):
    ...
    for file in list_file:
        if count_file < nb_frame:
            ...
            x[count_file,:,:,:] = get_x_frame(image_path, frame_id, wrg, hrg)
            count_file += 1

    return x, image_path

def get_y(path, snippet_id, ind, nb_frame, wrg, hrg, type, synset_wnet2id):
    ...
    for file in list_file:
        if count_file < nb_frame:
            frame_id = re.match(regexp,file).group(1)
            y[count_file, :, :, :] = get_y_frame(label_path, frame_id, wrg, hrg, synset_wnet2id)
            count_file += 1

    return y, label_path

def get_data(image_net_directory, batch_size, nb_frame, type, synset_wnet2id, data_augmentation = False, bucket_id = None, verbose = False):
    if type == 'train':
    ...
    for dir in batch:
        ...
        X[count_dir, :, :, :, :], image_path = get_x(path_x, snippet_id, ind, nb_frame, wrg, hrg, type)
        image_paths.append(image_path)
        if type != 'test':
            Y[count_dir, :, :, :, :], label_path = get_y(path_y, snippet_id, ind, nb_frame, wrg, hrg, type, synset_wnet2id)
